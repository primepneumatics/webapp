...
type Report = {
  ...
  service: { fab_number: string; model_number: string | null }
}
...
export function CustomerReports() {
  ...
  useEffect(() => {
    supabase.from('customers').select('name, org').eq('id', id).single().then(({ data }) => {
      if (data) setCustomerName(data.org || data.name)
    })
  }, [id])

  useEffect(() => {
    setLoading(true)
    supabase.from('services').select('id').eq('customer_id', id).then(({ data: services }) => {
      if (!services || services.length === 0) { setReports([]); setLoading(false); return }

      let query = supabase
        .from('service_reports')
        .select('id, report_number, report_date, total_run_hours, due_service_date, serviced_by, service:services(fab_number, model_number)')
  ...
  async function handleDownload() {
    ...
    setDownloading(true)
    await downloadPdf(printRef.current, `${customerName || 'customer'}-history.pdf`)
        ...
        <div ref={printRef}>
          <div className="hidden print:block mb-6">
            <h1 className="text-lg font-bold text-gray-900">
              Prime Pneumatics — Report History: {customerName}
            </h1>
            {(from || to) && (
          ...
          {loading ? (
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-12 text-center">
              <p className="text-gray-500 text-sm mb-4">No reports found for this period.</p>
              {!from && !to && (
                <Link to={`/reports/new/${id}`} className="inline-block px-4 py-2 bg-gray-900 text-white rounded-lg text-sm font-medium hover:bg-gray-800 transition-colors">
                  File First Report
                </Link>
              )}
            </div>
                ...
                {reports.map(r => (
                  <div key={r.id} className="bg-white border border-gray-200 rounded-xl p-4">
                    ...
                    <div className="grid grid-cols-2 gap-y-2 text-sm">
                      ...
                      <span className="text-gray-900 text-right font-mono">{r.service.fab_number}</span>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100 text-left">
                      ...
                      <th className="px-4 py-3 font-medium text-gray-600">Machine</th>
                  ...
                  <tbody>
                    {reports.map(r => (
                      <tr key={r.id} className="border-b border-gray-50 hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 font-mono text-xs text-gray-600">{r.service.fab_number}</td>
