...
export function ReportView() {
  ...
  async function downloadPdf(hideRates = false) {
  ...
  const reportCard = (
    <div ref={reportCardRef} className="bg-white p-8 space-y-6">
  ...
  // Non-own report: report card hidden off-screen, only download buttons visible
  return (
    <Layout>
      <div className="max-w-2xl">
        <div className="flex items-center justify-between mb-6">
          ...
          <div ref={printRef} className="relative">
            <button
              ...
              className="px-4 py-2 bg-gray-900 text-white rounded-lg text-sm font-medium hover:bg-gray-800 disabled:opacity-50 transition-colors flex items-center gap-1.5"
            >
              {downloading ? 'Downloading...' : 'Download'}
              {!downloading && <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor"><path d="M7 10l5 5 5-5z" /></svg>}
            </button>
            {printOpen && (
              <div className="absolute right-0 mt-1 w-44 bg-white border border-gray-200 rounded-xl shadow-lg z-20 overflow-hidden">
                <button
                  onClick={() => { setPrintOpen(false); downloadPdf(false) }}
                  className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50"
                >
                  Full Report
                </button>
                <button
                  onClick={() => { setPrintOpen(false); downloadPdf(true) }}
                  className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 border-t border-gray-100"
                >
                  No Rates
                </button>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-10 text-center text-sm text-gray-400">
          Select Full Report or No Rates above to download.
        </div>
      </div>

      {/* Hidden off-screen card used only for PDF capture */}
